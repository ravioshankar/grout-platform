...
const marketplaceRequest = Joi.object({
  /* TODO: add fields */
});
...
export default async function handler(req, res) {
  ...
  /* ---------- POST /marketplace ---------- */
  if (method === 'POST' && url.endsWith('/marketplace')) {
    try {
      const { error, value } = marketplaceRequest.validate(req.body);
  ...
  /* ---------- GET /marketplace/:id ---------- */
  if (method === 'GET' && url.startsWith('/marketplace/')) {
  ...
  /* ---------- PUT /marketplace/:id ---------- */
  if (method === 'PUT' && url.startsWith('/marketplace/')) {
    const id = url.split('/').pop();
    try {
      const { error, value } = marketplaceRequest.validate(req.body);
      if (error) throw new Error(error.message);
  ...
  if (method === 'DELETE' && url.startsWith('/marketplace/')) {
    const id = url.split('/').pop();
    try {
