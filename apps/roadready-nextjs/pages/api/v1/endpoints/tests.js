...
export default async function handler(req, res) {
  ...
  if (method === 'POST' && url.endsWith('/tests')) {
    try {
      const { error, value } = testsRequest.validate(req.body);
  ...
  /* ---------- GET /tests/:id ---------- */
  if (method === 'GET' && url.startsWith('/tests/')) {
  ...
  /* ---------- PUT /tests/:id ---------- */
  if (method === 'PUT' && url.startsWith('/tests/')) {
    const id = url.split('/').pop();
    try {
      const { error, value } = testsRequest.validate(req.body);
      if (error) throw new Error(error.message);
  ...
  /* ---------- DELETE /tests/:id ---------- */
  if (method === 'DELETE' && url.startsWith('/tests/')) {
    const id = url.split('/').pop();
    try {
