...
const email_verificationRequest = Joi.object({
  /* TODO: add fields */
});
...
export default async function handler(req, res) {
  ...
  if (method === 'POST' && url.endsWith('/email_verification')) {
    try {
      const { error, value } = email_verificationRequest.validate(req.body);
  ...
  if (method === 'GET' && url.startsWith('/email_verification/')) {
  ...
  /* ---------- PUT /email_verification/:id ---------- */
  if (method === 'PUT' && url.startsWith('/email_verification/')) {
    const id = url.split('/').pop();
    try {
      const { error, value } = email_verificationRequest.validate(req.body);
      if (error) throw new Error(error.message);
  ...
  /* ---------- DELETE /email_verification/:id ---------- */
  if (method === 'DELETE' && url.startsWith('/email_verification/')) {
    const id = url.split('/').pop();
    try {
